...
!!! question
    ...
    === "Answer 2"
         The genome size for our assembly 4.4mb, which is identical to the 4.4mb of mtb. It is advised that you look at a variety of metrics and supporting analyses to determine quality of assembly. 
...
```
flye --nano-hq tb_ONT/sample1_ONT.fastq.gz --genome-size 4.4m --threads 2 --read-error 0.06 -o long/
```
...
### (Optional) Compare against a Canu assembly

**THIS TAKES A LONG TIME TO RUN SO FINISH THE OTHER PARTS OF THE PRACTICAL FIRST**

Flye gave you one long-read assembly. As an optional extension, you can run a
**second** assembler, [Canu](https://canu.readthedocs.io/en/latest/quick-start.html#assembling-pacbio-clr-or-nanopore-data) on the same reads and compare the two with QUAST.

Before you look at the command, try to write it yourself from the Canu docs.
You'll need to tell Canu four things: an output prefix, an output directory,
the genome size (what's the *M. tuberculosis* genome size?), and the read
technology. Have a go, then expand below to check.

<details>
<summary>💡 Click to reveal a worked Canu command</summary>

​```bash
 mamba create -n canu22 -y -c conda-forge -c bioconda     "canu=2.2" "python=3.10"

canu -assemble     -p sample1 -d long_canu2/     genomeSize=4.4m maxThreads=4     useGrid=false     -nanopore-corrected tb_ONT/sample1_ONT.fastq.gz
​```

When it finishes, the assembly is at `long_canu/sample1.contigs.fasta`, the
Canu equivalent of Flye's `assembly.fasta`. Run it through QUAST alongside the
Flye assembly to compare contiguity and completeness.
...
```
racon -t 2 tb_ONT/sample1_ONT.fastq.gz long/racon.paf long/tgs_gapcloser.scaff_seqs.fa > long/final_assembly.fa
```
!!! question
    === "Question 7"
